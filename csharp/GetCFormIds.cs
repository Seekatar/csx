...
var files = Directory.EnumerateFiles("/Users/jwallace/temp/cforms-localhost", "*.json");
// var files = Directory.EnumerateFiles("/Users/jwallace/temp/cforms-localhost", "1f0109a7-8232-428a-aa34-da7438682a8e.json");
// var files = Directory.EnumerateFiles("/Users/jwallace/temp/cforms", "*.json");
foreach( var file in files)
{
    Console.WriteLine($">>>> Processing {file}");
    new JsonReader().FindElementsByName(file, elements);
}
...
foreach (var element in elements.OrderByDescending(o => o.Value.Item1))
{
    ...
    Console.WriteLine($"{i++,3} {element.Key}: {element.Value.Item1} {element.Value.Item2} '{value}'");
}
...
public class JsonReader
{
    ...
    public void FindElementsByName(string filePath, Dictionary<string, (int,string)> elements)
    {
        var thisFile = new HashSet<string>();
        var json = JsonNode.Parse(File.ReadAllText(filePath));
        var steps = json; // ["steps"];
        FindElements(steps, elements, thisFile);
    }

    private void FindElements(JsonNode node, Dictionary<string, (int,string)> elements, HashSet<string> thisFile)
    {
        if (node is JsonObject jsonObject)
        {
            foreach (var property in jsonObject)
            {
                ...
                FindElements(property.Value, elements, thisFile);
            }
        }
        else if (node is JsonArray jsonArray)
        {
            ...
            {
                FindElements(item, elements, thisFile);
            }
        }
    }
}
