...
int age2(DateTime birthdate, DateTime? today = null)
{
    WriteLine("age");
    return 1;
}
...
when = DateTime.Parse("12-01-2022");
WriteLine( $"{date:d} on {when:d} {age(date, when)}");
...
// WriteLine($"4 {age2(IfNull(x,DateTime.Today))}");
//WriteLine($"4 {age2(OldIfNull(d,DateTime.Today))}");
WriteLine($"4 {age2(NewIfNull(d,DateTime.Today))}");
WriteLine($"4 {age2(IfNull2(d,DateTime.Today))}");